...
logger = setup_logger(name="cs_tracker.item_extractor")
...
class ItemExtractor:
        ...
        try:
            ...
            for idx, row in enumerate(rows):
                ...
                if len(cells) >= 6:
                    ...
                    logger.debug(
                        f"Fila {idx} descartada: solo tiene {len(cells)} celdas"
                    )
            ...
            if len(valid_rows) == 0 and len(rows) > 0:
                logger.warning(
                    f"⚠️ Ninguna fila tiene >= 6 celdas. Intentando selector alternativo..."
                )
    ...
    async def _extract_single_item(self, row, idx: int, timestamp: str) -> Dict:
        try:
            ...
            if not item_data:
                if idx < 3:
                    logger.debug(
                        f"Fila {idx}: _extract_item_name_and_url devolvió None"
                    )
                return None
    ...
    async def _extract_platform_url(self, cells, cell_idx: int, url_pattern: str):
        try:
            ...
            logger.debug(
                f"No se pudo extraer URL {url_pattern} de celda {cell_idx}: {e}"
            )
            return None

    async def _extract_price_data(
        self, cells, cell_idx: int, row_idx: int, platform: str
    ):
        try:
            cell = cells[cell_idx] if len(cells) > cell_idx else None
            if not cell:
    ...
    async def _extract_text_from_cell(self, cells, cell_idx: int, row_idx: int):
        try:
            ...
            logger.debug(
                f"No se pudo extraer texto de celda {cell_idx} en fila {row_idx}: {e}"
            )
