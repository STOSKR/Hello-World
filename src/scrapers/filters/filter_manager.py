...
logger = setup_logger(name="cs_tracker.filter_manager")
...
class FilterManager:
    ...
    async def configure_all_filters(self, page: Page):
        ...
        # 3. Configurar modo de compra (si aplica)
        await self._configure_buy_mode(page)

        # 4. Configurar tipo de balance
        await self._configure_balance_type(page)

        # 5. Configurar filtros de precio y volumen
        await self._configure_price_volume_filters(page)

        # 6. Configurar plataformas
            ...
            if await currency_selector.count() > 0:
                ...
                if await currency_option.count() > 0:
                    ...
                    await page.wait_for_timeout(
                        3000
                    )  # Esperar a que recargue los precios
                else:
                    logger.warning(f"No se encontró opción {currency_code} en el menú")
                    await page.keyboard.press("Escape")
            else:
                logger.info(
                    "La moneda ya está configurada o no se encontró el selector"
                )
    ...
    async def _configure_buy_mode(self, page: Page):
        try:
            buy_mode = self.config.get("price_mode.buy_mode")
            if not buy_mode:
                logger.info("No se configuró buy_mode, omitiendo...")
                return

            logger.info(f"Verificando opción '{buy_mode}'...")
            ...
            if await buy_tab.count() > 0:
                ...
                if "active" not in tab_class:
                    ...
                    await page.wait_for_timeout(1000)
                    logger.info(f"✅ '{buy_mode}' seleccionado")
                else:
                    logger.info(f"✅ '{buy_mode}' ya estaba seleccionado")
        except Exception as e:
            logger.warning(f"Error al configurar modo de compra: {e}")
            ...
            for platform, value in [
                ...
                try:
                    ...
                    if await checkbox.count() > 0:
                        ...
                        should_be_checked = self.config.get(
                            f"platforms.{platform}", False
                        )
                        ...
                        if is_checked != should_be_checked:
                            ...
                            logger.info(
                                f"{platform} ya estaba {'marcado' if is_checked else 'desmarcado'}"
                            )
                except Exception as e:
